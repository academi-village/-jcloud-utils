...
import static com.github.academivillage.jcloud.errors.ProjectError.ACCESS_DENIED;
import static com.github.academivillage.jcloud.errors.ProjectError.USER_NOT_AUTHENTICATED;
import static com.github.academivillage.jcloud.util.dynamikax.security.VeryCompactJwtPermissions.*;
...
public class SimpleAuthorizationService implements AuthorizationService {
    ...
    public void checkAccess(Permission... permissions) {
        val globals     = (List<String>) extractPermissions(getClaims()).get(GLOBALS);
    ...
    public void checkAccess(long studyId, Permission... permissions) {
        val activitiesMap = (List<Map>) extractPermissions(getClaims()).get(ACTIVITIES);
        activitiesMap.stream()
                .filter(jwtAcc -> ((Integer) jwtAcc.get(PROJECT_ID)).longValue() == studyId)
                .map(jwtAcc -> (List<String>) jwtAcc.get(ACTIVITIES))
    ...
    public Optional<String> getOptionalJwtToken() {
        ...
        if (attributes == null) {
            ...
            log.warn("ServletRequestAttributes is null. It seems you called this from a non-request bounded thread", ex);
